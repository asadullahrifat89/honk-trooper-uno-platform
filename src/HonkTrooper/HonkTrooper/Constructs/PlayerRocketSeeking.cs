...
namespace HonkTrooper
{
    public partial class PlayerRocketSeeking : BombSeeking
    {
            ...
            double downScaling)
        {
            ...
            _rocket_launch_sounds = Constants.SOUND_TEMPLATES.Where(x => x.SoundType == SoundType.SEEKER_ROCKET_LAUNCH).Select(x => x.Uri).Select(uri => new Sound(uri: uri, volume: 0.5)).ToArray();
